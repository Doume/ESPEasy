...
void WifiAPconfig()
{
  ...
  char ap_ssid[26];
  memset(ap_ssid, 0x00, sizeof(ap_ssid));

  // AP-Mode : SSID = Name if defined.
  if (strlen(Settings.Name) > 0)
  {
    strncpy(ap_ssid, Settings.Name, (strlen(Settings.Name) <= sizeof(ap_ssid) ? strlen(Settings.Name) : sizeof(ap_ssid)));
  }
  else
  {
    strcpy(ap_ssid, "ESP_");
